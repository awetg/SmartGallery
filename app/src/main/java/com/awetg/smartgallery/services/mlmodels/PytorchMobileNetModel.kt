...
class PytorchMobileNetModel(private val context: Context, assetName: String) {

    private val module by lazy {
        LiteModuleLoader.load(getAbsolutePathOfAssetFile(context, assetName))
    }

    fun getClassification(bitmap: Bitmap): List<String> {
        ...
        // get tensor output as float array
        val scores = outputTensor.dataAsFloatArray

        // get the max value out of the scores, we will only find 1 class for each image
        ...
        if (maxScore >= 0) {
            return listOf(IMAGENET_CLASSES[maxScoreIdx])
        }
        return emptyList()
    }
